...
- Use MCP tools (e.g. `run_newsboat`, `run_kw`) instead of running those commands directly via Bash. The MCP wrappers exist to bypass sandbox restrictions reliably.
    ...
    * Use `run_dic` to speak text aloud via the `dic` TTS wrapper. Accepts `text`, optional `voice` (default: bf_isabella), and optional `speed`.

## Making Config Changes

- When modifying configuration, MCPs, etc., always make changes in `~/Repos/dotfiles` — never edit the deployed copy directly.

## Prompts

- Frequently used prompts live in `~/.agents/prompts/*.md`. If asked to study something or reference something, look there.
