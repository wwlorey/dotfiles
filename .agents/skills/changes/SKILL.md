...
- **Worker dies mid-implement.** Before dequeueing the next approved item, check `git status`. If the tree is dirty (uncommitted changes from the failed worker), do not start the next item. Surface the dirty tree to the user with the failing item's id and let them decide: clean up and re-spawn the failed item, finish manually, or revert and reclassify the item as `blocked`. Starting a new worker on a dirty tree compounds the failure.
