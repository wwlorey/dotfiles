...
> 7. Commit (code + specs + issue) with an imperative <72-char message. **Stage explicit paths only:** other agents are working in this same tree right now, so `git add <path>` each file you changed, by name — never `git add -A`, `git add .`, or `git add -u`. `git status` will show files you did not touch; leave them alone. Push. If push fails, report and continue — the commit is safe locally.
>
