...
4. Check your invocation context for a caller-supplied `## Per-item gate policy (from dev)` section — its bullets populate the worker briefing's "Required verification gates" section (per the `orchestrate` skill's "Required verification gates" briefing-checklist item). If absent, omit that section from the briefing.
...
**`adversarial-verify` is a fresh-context spawn.** When a policy bullet names `adversarial-verify`, the gate worker must be a brand-new agent that did not implement any iteration in the batch — the implementer can never be its own adversary, so this gate never rides along in an iteration worker's briefing. The bullet carries the guarantee in plain English and the surface it lives on; put both in the briefing along with the slate's commit range. When a spec covers the surface, take the guarantee and its violation attempt verbatim from that spec's `### Absolutes register` rather than restating them — the register is where the claim and the attempt that would disprove it are already written down, and paraphrasing is how an adversary ends up testing something narrower than the claim. Instruct the worker to: attempt the violation from every route the code permits (scripted invocation instead of the interactive path, the CLI / IPC / API entry point called directly, hostile arguments, two callers at once); probe the boundary between the new code and its substrate — the database (concurrent writers, lock contention, transaction scope, preconditions built on clocks instead of versions), IPC, the filesystem, and ordering assumptions; and treat the green suite as its starting position rather than as evidence. Require the same INTRODUCED-BY-SLATE / PRE-EXISTING tagging and `## Trackers to file` block as any other gate worker.
...
>    - If the issue's `type` is `bug`: reproduce → write a failing test that captures the bug → fix → re-run the test until it passes. When the buggy code parses or validates artifacts produced by an external system (platform receipts, third-party API responses, foreign file formats), the failing test uses a **captured real artifact** as its fixture wherever one exists or can be captured — a synthetic fixture built from the same understanding the code was written from can pass while the code stays wrong about the external system.
...
>    - **Adversary tests are part of acceptance, not extra credit.** When the issue claims a guarantee — a refusal, a confirmation or authorization boundary, an invariant, a limit, a durability or integrity claim, or any behavior stated with *cannot* / *never* / *always* / *only* — the issue is not done until at least one test *attempts the forbidden thing and asserts it is refused*, reached the way someone bypassing the feature would reach it (scripted call instead of the interactive prompt, the entry point invoked directly, hostile arguments) rather than only the intended path. Happy-path coverage of a guarantee proves the guarantee works when honored, which is not the claim.
>    - **Probe the substrate boundary.** Defects cluster where new code meets concurrency, transactions, IPC boundaries, filesystem paths, and clock / ordering assumptions: two writers racing, a lock already held, a precondition checked against a timestamp instead of a version, a path that escapes its root. Cover the boundaries this issue's code actually touches — a wrong error code, a stale read left confirmable, or a silently lost update is a defect even when every other test is green.
...
> 8. Close: edit frontmatter `status: in_progress` → `status: closed`. Add a `## Comments` entry summarizing what was done. If the issue is a **field-reported bug** whose original failing scenario cannot be re-run from here (distributed build, user's machine, external environment), close per the `issues` skill's field-reported rule: the comment says "fix attempted, field verification pending", a companion `<slug>-field-verify` tracker (`status: open`) exists before you close, and neither the commit message nor the comment describes the change as "fixed."
...
> - Never describe a guarantee as holding, enforced, or safe on the strength of tests you wrote. Your suite encodes your own mental model of the code. In the close-comment and the structured return, say what you implemented and which adversary tests you added; an independent `adversarial-verify` agent, spawned by the orchestrator, decides whether the guarantee survives.
...
>
> **Required verification gates.** [The orchestrator populates this section from the caller-supplied `## Per-item gate policy (from dev)` section, or omits it entirely when no caller policy applied. `adversarial-verify` never appears here — it requires an agent that did not write the code, so the orchestrator fires it separately.]
>
> **Return format.**
> ```
> ## Slug
...
- **Session-close gates.** Look in your invocation context for a `## Session-close gate policy (from dev)` section. If present, AND the aggregated `## New work surfaced` section contains nothing that would queue more iterations into the current session, fire those session-close gates now. Surface findings in the report.
  ...
  - **Nothing guaranteed ships unverified.** Fire `adversarial-verify` (fresh-context spawn, per the mid-iteration checkpoint section) for every guaranteed surface this run touched that has changed since an independent agent last tried to break it — a remediation commit counts as a change. Do not report the run complete — or describe any guarantee as holding — while one is outstanding.
