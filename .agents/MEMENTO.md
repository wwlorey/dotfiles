...
- **Post-commit push prompt** — after every commit, ask the user "Push to origin?" rather than waiting for them to bring it up. Do not push without an explicit yes. Exception: skills or pipelines that script the push as part of their procedure (e.g. `build`'s iteration-end push) override this rule for the commits they make.
